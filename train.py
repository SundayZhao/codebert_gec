...
from tools.datareader import Seq2EditDatasetReader
from tools.seq2edit_model import Seq2Edit
...
def build_data_loaders(data_set: AllennlpDataset,batch_size: int,num_workers: int,shuffle: bool,batches_per_epoch = None):

    return PyTorchDataLoader(data_set, batch_size=batch_size, num_workers=num_workers, shuffle=shuffle,collate_fn=allennlp_collate, batches_per_epoch=batches_per_epoch)
...
def get_data_reader(model_name, max_len, skip_correct=False, skip_complex=0,
    ...
    reader = Seq2EditDatasetReader(token_indexers=token_indexers,
                                     max_len=max_len,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights_name',
                        type=str,
                        default="Path to the bert encoder modelrt")  
