...
class GecBERTModel(object):
    def __init__(self,
        ...
        for model_path, weights_name in zip(model_paths, weights_names):  
            ...
            model = Seq2Edit(vocab=self.vocab,
    ...
    def preprocess(self, token_batch):
        with torch.cuda.device(self.cuda_device):
            seq_lens = [len(sequence) for sequence in token_batch if sequence]  # 长度
            if not seq_lens:
                return []
    ...
    def postprocess_batch(self, batch, all_probabilities, all_idxs,error_probs, all_d_tags_idxs, max_len=50):
        all_results = []
        noop_index = self.vocab.get_token_index("$KEEP", "labels")  
        for tokens, probabilities, idxs, error_prob, d_tags_idxs in zip(batch,all_probabilities,all_idxs,error_probs,all_d_tags_idxs):
    ...
    def handle_batch(self, full_batch):
        ...
        for n_iter in range(self.iterations):
            ...
            pred_batch = self.postprocess_batch(orig_batch, probabilities,idxs, error_probs, d_tags_idxs, self.max_len)
            if self.log:
